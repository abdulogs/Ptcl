...
<?php // Listing
  ...
  $listing = DB::columns(["c.id","c.message","c.reply","c.status","c.created_at", "c.updated_at"]);
  $listing = DB::columnsmore(["u.first_name","u.last_name","u.email","u.phone","u.address"]);
  $listing = DB::table("compliants as c");
  $listing = DB::join(["users AS u" => ["u.id" => "c.user_id"]],"LEFT");
  $listing = DB::order("c.id", "DESC");
                ...
                <?php if($listing):?>
                  <?php foreach($listing as $data):?>
                    <tr>
                        ...
                        <td class="align-middle text-break" data-label="Address">
